...
export default function AddVehiclePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await vehicleService.create(payload, token);
      if (response.success) {
    ...
    <div className="max-w-5xl mx-auto space-y-12 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-heading font-bold tracking-tight">Add New Vehicle</h1>
          <p className="text-muted-foreground">Expand your fleet with a new premium vehicle</p>
        </div>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 bg-background/80 backdrop-blur-xl border border-border/40 p-3 rounded-3xl shadow-2xl flex items-center gap-4 min-w-[320px] justify-between">
         <div className="flex items-center gap-3 ml-2">
            ...
            <span className="text-sm font-bold truncate max-w-[150px]">
                {formData.vehicle_name || "New Vehicle"}
            </span>
         </div>
         <div className="flex gap-2">
            <Button variant="ghost" size="sm" onClick={() => router.push("/dashboard/vehicles")} className="rounded-xl font-bold">Discard</Button>
            <Button size="sm" onClick={handleSubmit} disabled={isLoading} className="rounded-xl font-bold px-6">
